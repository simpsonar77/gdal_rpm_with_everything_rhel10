...
CUSTOM BUILD by Andrew Simpson - Enables all standard drivers/plugins.
-- creates non-versioned symlinks for the jar files.
-- by default, creates gdal-java subpackage
...
%if %{with java}
# Create unversioned symlinks for java files
ln -s gdal-%{version}.jar %{buildroot}%{_jnidir}/%{name}/gdal.jar
ln -s gdal-%{version}-sources.jar %{buildroot}%{_jnidir}/%{name}/gdal-sources.jar
ln -s gdal-%{version}.pom %{buildroot}%{_jnidir}/%{name}/gdal.pom
%endif
...
%{_jnidir}/%{name}/gdal.jar
%{_jnidir}/%{name}/gdal-sources.jar
%{_jnidir}/%{name}/gdal.pom
...
%changelog
* Wed Apr 29 2026 Andrew Simpson <andrew.r.simpson6.civ@us.navy.mil> - 3.10.3-4
- Rebuild from RHEL 10.0 Source.
- This had to be done because RHEL is full of clowns and removed most of the drivers
- Update to 3.10.3
